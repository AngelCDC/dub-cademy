    ...
    <div className="bg-surface min-h-screen">
      ...
      <div className="relative bg-white dark:bg-card border-b border-violet-100 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 py-16 md:py-20 relative">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-ink tracking-tight mb-4">Aprende en comunidad</h1>
          ...
          <div className="flex flex-wrap gap-6 text-sm text-slate-500">
            {[{ v: "500K+", l: "Miembros" }, { v: "50K+", l: "Hilos activos" }, { v: "24/7", l: "Actividad" }].map((s) => (
              <div key={s.l} className="flex items-center gap-2">
                <span className="font-bold text-ink">{s.v}</span>
                <span>{s.l}</span>
              </div>
            ))}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          {/* Categories */}
          <div>
            <h2 className="text-lg font-bold text-ink mb-4">Categorías</h2>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {CATEGORIES.map((cat) => (
                <div key={cat.name} className="bg-white dark:bg-card border border-violet-100 hover:border-violet-300 rounded-2xl p-4 cursor-pointer transition-all hover:shadow-md hover:shadow-violet-50 group">
                  <div className="text-2xl mb-2">{cat.icon}</div>
                  <h3 className="text-xs font-semibold text-ink leading-snug mb-1 group-hover:text-primary transition-colors">{cat.name}</h3>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-ink">Discusiones recientes</h2>
              <span className="text-xs text-slate-400">Actualizado en tiempo real</span>
            </div>
            <div className="space-y-2">
              {THREADS.map((thread) => (
                <div key={thread.title} className="bg-white dark:bg-card border border-violet-100 hover:border-violet-200 rounded-2xl p-5 cursor-pointer transition-all hover:shadow-sm group">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="text-sm font-semibold text-ink group-hover:text-primary transition-colors leading-snug">{thread.title}</h3>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-gradient-to-br from-primary to-fuchsia-600 rounded-2xl p-6 text-white">
            ...
            <a href="#" className="flex items-center justify-center gap-2 bg-white dark:bg-card text-primary font-semibold text-sm py-3 rounded-full hover:bg-white/90 dark:hover:bg-background/90 transition-colors">
              Entrar al servidor <ArrowRight className="size-4" />
            </a>
          </div>

          {/* Stats */}
          <div className="bg-white dark:bg-card border border-violet-100 rounded-2xl p-5">
            <h3 className="text-sm font-bold text-ink mb-4">En la zona ahora</h3>
          ...
          {/* Tags */}
          <div className="bg-white dark:bg-card border border-violet-100 rounded-2xl p-5">
            <h3 className="text-sm font-bold text-ink mb-4">Tags populares</h3>
