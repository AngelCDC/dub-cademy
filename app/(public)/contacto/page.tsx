    ...
    <div className="bg-surface min-h-screen">
      ...
      <div className="relative bg-white dark:bg-card border-b border-violet-100 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 py-16 md:py-20 relative">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-ink tracking-tight mb-4">Hablemos</h1>
      ...
      <div className="max-w-7xl mx-auto px-6 py-14 md:py-20 grid grid-cols-1 lg:grid-cols-5 gap-12">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div>
            <h2 className="text-lg font-bold text-ink mb-1">Contacto directo</h2>
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[Linkedin, Twitter, Youtube, MessageCircle].map((Icon, i) => (
                <a key={i} href="#" className="size-9 rounded-xl bg-white dark:bg-card hover:bg-violet-50 border border-violet-100 flex items-center justify-center text-violet-400 hover:text-primary transition-all">
        ...
        <div className="lg:col-span-3">
          <div className="bg-white dark:bg-card border border-violet-100 rounded-2xl p-7 md:p-8 shadow-sm shadow-violet-50">
            <h3 className="text-base font-bold text-ink mb-1">Envíanos un mensaje</h3>
            ...
            <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                ...
                {[["nombre", "Nombre", "Juan"], ["apellido", "Apellido", "García"]].map(([id, label, ph]) => (
                  <div key={id}>
                    ...
                    <input type="text" id={id} required placeholder={ph}
                      className="w-full bg-surface border border-violet-100 hover:border-violet-200 focus:border-primary/50 focus:bg-white dark:focus:bg-muted rounded-xl px-4 py-3 text-sm text-ink placeholder:text-slate-300 outline-none transition-colors"
                    />
                  </div>
                ))}
              </div>
              ...
              <div>
                ...
                <select id="asunto" required
                  className="w-full bg-surface border border-violet-100 hover:border-violet-200 focus:border-primary/50 focus:bg-white dark:focus:bg-muted rounded-xl px-4 py-3 text-sm text-ink outline-none transition-colors appearance-none"
                >
              ...
              <div>
                ...
                <textarea id="mensaje" required rows={5} placeholder="Cuéntanos en qué podemos ayudarte…"
                  className="w-full bg-surface border border-violet-100 hover:border-violet-200 focus:border-primary/50 focus:bg-white dark:focus:bg-muted rounded-xl px-4 py-3 text-sm text-ink placeholder:text-slate-300 outline-none transition-colors resize-none"
                />
              </div>
      ...
      {/* FAQ */}
      <div className="bg-white dark:bg-card border-t border-violet-100 py-16 md:py-24">
        <div className="max-w-3xl mx-auto px-6">
          <h2 className="text-2xl md:text-3xl font-bold text-ink mb-10 text-center">Preguntas frecuentes</h2>
          <div className="space-y-2">
            {FAQS.map((faq, i) => (
              <div
                key={i}
                className="bg-surface border border-violet-100 hover:border-violet-200 rounded-2xl overflow-hidden cursor-pointer transition-colors"
                onClick={() => setActiveFaq(activeFaq === i ? null : i)}
              >
                <div className="flex items-center justify-between gap-4 px-6 py-4">
                  <span className={`text-sm font-semibold transition-colors ${activeFaq === i ? "text-primary" : "text-ink"}`}>
                    {faq.q}
                  </span>
