...
export function BuscarClient({ courses }: { courses: PublicCourseType[] }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 pt-20 pb-16">
      {/* Big search input */}
      <div className="mb-10">
        <div className="flex items-center gap-3 bg-white dark:bg-card border-2 border-violet-100 focus-within:border-primary/50 rounded-2xl px-5 py-4 shadow-sm focus-within:shadow-md focus-within:shadow-primary/10 transition-all">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-base text-ink placeholder:text-slate-300 outline-none"
          />
          {query && (
      ...
      {!query.trim() && (
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-12">
            {POPULAR.map((term) => (
              <button
                ...
                className="flex items-center gap-1.5 bg-white dark:bg-card border border-violet-100 hover:border-violet-300 hover:bg-violet-50 text-slate-600 hover:text-primary text-sm px-4 py-2 rounded-full transition-all"
              >
          ...
          <div className="bg-white dark:bg-card border border-violet-100 rounded-2xl p-8 text-center">
            ...
            <p className="text-ink font-semibold mb-1">Empieza a buscar</p>
      ...
      {query.trim() && (
        <>
          <p className="text-sm text-slate-500 mb-6">
            {results.length > 0
              ? <><span className="font-semibold text-ink">{results.length} resultado{results.length !== 1 ? "s" : ""}</span> para &ldquo;{query}&rdquo;</>
              : <>Sin resultados para &ldquo;{query}&rdquo;</>
            }
          </p>

          {results.length === 0 ? (
            <div className="bg-white dark:bg-card border border-violet-100 rounded-2xl p-12 text-center">
              <div className="size-14 rounded-2xl bg-violet-50 flex items-center justify-center mx-auto mb-4">
                <Search className="size-6 text-violet-300" />
              </div>
              <p className="text-ink font-semibold mb-1">Sin coincidencias</p>
